...
function extractMangaUrlsFromHtml(html: string) {
  const escapedBaseUrl = escapeForRegex(sourceDefinition.baseUrl)
  const matches = html.match(
    new RegExp(`${escapedBaseUrl}\\/manga\\/[^"'\\\\\\s<]+\\/?`, "g")
  )
...
function extractImageUrlsFromChapterHtml(html: string) {
  const host = escapeForRegex(new URL(sourceDefinition.baseUrl).hostname)
  const matches = html.match(
    new RegExp(
      `https:\\/\\/${host}\\/wp-content\\/uploads\\/WP-manga\\/data\\/[^"'\\\\\\s>]+\\.(?:jpg|jpeg|png|webp)`,
      "gi"
    )
  )
...
function extractCoverImage(html: string) {
  ...
  const matches = html.match(
    new RegExp(
      `https:\\/\\/${host}\\/wp-content\\/uploads\\/[^"'\\\\\\s>]+\\.(?:jpg|jpeg|png|webp)`,
      "gi"
    )
  )
...
function extractMangaDetailsFromHtml(
  ...
  const title =
    getMetaContent(html, "og:title")?.replace(/\s+[â€“-]\s+BrainRotComics$/i, "") ||
    extractBetween(html, /<title>([\s\S]*?)<\/title>/i).replace(
      /\s+[â€“-]\s+BrainRotComics$/i,
      ""
    ) ||
    toTitleCase(slug)
  ...
  return {
    ...
    genres: genres.length ? genres : [sourceDefinition.label],
...
function toPreview(info: SourceMangaInfo): SourceMangaPreview {
  ...
  return {
    ...
    genres: info.genres ?? [sourceDefinition.label],
    status: info.status,
...
const getSourceMangaInfoByUrl = cache(
  async (url: string): Promise<SourceMangaInfo | null> => {
    ...
    return {
      ...
      sourceName: sourceDefinition.label,
    }
  }
)

const getHomepageManga = cache(async (limit = 18) => {
  const html = await fetchHtml(sourceDefinition.baseUrl)
...
async function browse(filters: SourceBrowseFilters = {}): Promise<SourceBrowseResult> {
  const limit = filters.limit ?? 12

  if (!filters.q?.trim()) {
    const homepageItems = await getHomepageManga(limit)
...
async function getMangaInfoBySlug(slug: string) {
  return getSourceMangaInfoByUrl(`${sourceDefinition.baseUrl}/manga/${slug}/`)
}
...
export const aryaScansSource: SourceAdapter = {
